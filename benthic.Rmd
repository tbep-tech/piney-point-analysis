...
Number of samples per year, historical and 2021.
...
ggplot(toplo, aes(x = yr)) + 
  ...
  scale_x_continuous(breaks = seq(min(toplo$yr), max(toplo$yr)), expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) + 
  coord_cartesian(ylim = c(0, NA)) +
  thm
```

TBBI categories by year, historical and 2021.

```{r, fig.width = 9, fig.height = 4}
toplo <- bntdat 

ggplot(toplo, aes(x = yr)) + 
  geom_bar(stat = 'count', alpha = 0.4, aes(fill = TBBICat), colour = 'grey') + 
  labs(
    y = 'Samples'
  ) +
...
TBBI average scores by year, historical and 2021.  Only includes years with > 2 samples.  Colors are breakpoints for TBBI categories.
