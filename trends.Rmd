...
# monitoring data
rswqtmp <- rswqdat %>% 
  filter(var %in% c('chla', 'tn', 'secchi')) %>% 
  ...
  mutate(
    avev = case_when(
      var %in% c('chla', 'tn') ~ Gmean(val), 
      T ~ mean(val)
    ),
    valhi = case_when(
      var %in% c('chla', 'tn') ~ as.numeric(Gmean(val, conf.level = 0.95)[3]),
      T ~ as.numeric(t.test(val)$conf.int[2])
    ),
    vallo = case_when(
      var %in% c('chla', 'tn') ~ as.numeric(Gmean(val, conf.level = 0.95)[2]),
      T ~ as.numeric(t.test(val)$conf.int[1])
    )
  ) %>% 
  ungroup() %>% 
  select(yr, var, avev, vallo, valhi) %>% 
  unique() %>% 
  mutate(
    var = factor(var, levels = c('tn', 'chla', 'secchi'),
                 labels = c(
...
ggplot(rswqtmp,  aes(x = factor(yr), y = avev, fill = factor(yr))) + 
  with_shadow(geom_bar(stat = 'identity'), sigma = 2.7, x_offset = 0, y_offset = 0) + 
  geom_errorbar(aes(ymin = vallo, ymax = valhi), width = 0) + 
  facet_wrap(~var, scales = 'free_y', labeller = label_parsed, strip.position = 'left') + 
  ...
  theme(
    ...
    panel.background = element_blank(), #element_rect(fill = 'grey95', color = 'white'), 
  ...
  labs(
    ...
    y = NULL, 
    fill = NULL,
    title = 'Water Quality August and September'
  )
```
